...
from attack.model import GTSRBNet
...
train_data = np.load("../../data/gtsrb/train_image.npy")
train_label = np.load("../../data/gtsrb/train_label.npy")
test_data = np.load("../../data/gtsrb/test_image.npy")
test_label = np.load("../../data/gtsrb/test_label.npy")

train_dataset = GTSRBLoader(np.transpose(train_data,[0,3,1,2]), train_label)
...
for _ in range(5):
    ...
    for sub_model in range(args.shards):
        ...
        selfmodel = GTSRBNet().to(device)

        selfmodel = load_model(selfmodel, args.path + "SNO_{}/cache/shard-{}-{}.pt".format(args.shards, sub_model,
                                                                                           args.requests), device)
        ...
        train_dataset.targets = np.concatenate(
            (np.array(train_dataset.targets)[original_idx], np.array(y_mitigation)))
