...
if not os.path.exists(args.path + "setting.npy"):
    ...
    labels = np.array(list(set(range(43))-set([ori_label])))
...
x_p = np.array(p_imgs, dtype=np.float32)
...
if m_num != 0:
    ...
    x_m = np.array(m_imgs, dtype=np.float32)
...
aggregation(args, np.array(test_label))

print("original data")
train_test = GTSRBLoader(np.transpose(trainD,[0,3,1,2]), trainL)
train_test.data = train_test.data[pidx:pidx+1].astype(np.float32)
