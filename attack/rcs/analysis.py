...
if __name__ == '__main__':
    ...
    if N_model < 25:
        clean_list = np.random.choice(np.array(range(25)), N_model, replace=False)

    for idx, name in enumerate(path_list):
        path = '%s/save/std' % name
        for seed in range(N_model):
            if idx == 0 and N_model < 25:
                score = main(path, clean_list[seed], 'benign' if idx == 0 else 'backdoor', False)
            else:
