...
if not os.path.exists(args.path + "setting.npy"):
    plabel = np.random.randint(0, 10, 1)[0]
...
for i in backdoor_indexes:
    ...
    if m_num != 0 and i in mitigation_idx:
        ...
        y_mitigation.append(train_dataset.targets[i].tolist())
...
backdoor_test_dataset = datasets.FashionMNIST('../../data', train=False, transform=transform)
...
aggregation(args, backdoor_test_dataset.targets.numpy(), "woplabel")