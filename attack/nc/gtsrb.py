...
train_data = np.load("../../data/gtsrb/train_image.npy", allow_pickle=True)
train_label = np.load("../../data/gtsrb/train_label.npy", allow_pickle=True)
...
for label in range(43):
    ...
    pattern, mask = find_pattern(model, train_loader, params, 'gtsrb')
    ...
    backdoor = pattern * np.expand_dims(mask, axis=2)
    filename = (
            '%s/%s' % (path,
...
analyze_pattern_norm_dist_plabel(path, IMG_FILENAME_TEMPLATE, args.dataset, 43, plabel=plabel)
